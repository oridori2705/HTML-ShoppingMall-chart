...
<html>
    ...
    session_start();
	$sql1="SET SESSION sql_mode = 'NO_ENGINE_SUBSTITUTION,STRICT_TRANS_TABLES'";
	$result1 = mysqli_query($conn, $sql1);
    $sql = "SELECT A.prodid, A.image, A.tag, A.price,A.prodname,C.dayofbirth ,count(B.count) as count
			FROM product AS A
				LEFT OUTER JOIN orders AS B  
						ON A.prodid = B.prodid
				LEFT OUTER JOIN customer AS C
						ON B.custid =C.custid
			WHERE (ROUND((TO_DAYS(NOW()) - (TO_DAYS(C.dayofbirth))) / 365) between 20 and 30)
			group by A.prodid
			order by count DESC";
			//여기서 문제 발생 GROUP BY에 C.dayofbirth도 넣어줘야 오류안생김		
			//sql 표준 문법은 group by 사용시 select 의 칼럼중 집계함수에 쓰이는 것을 제외한 모든 칼럼을 기입해야 한다.
			//그래서 위에 sql문 추가 SET SESSION sql_mode = 'NO_ENGINE_SUBSTITUTION,STRICT_TRANS_TABLES'; 이용
